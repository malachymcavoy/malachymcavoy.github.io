...
__Estimated percentage who think that global warming is caused mostly by human activity__

‘‘‘{r, echo = TRUE, eval = TRUE, fig . align='center’,}
    cc_belief_county <− read.table(
      ’https://bcecon.github.io/cc_belief_county.csv’,
      sep = ',', header = TRUE, stringsAsFactor = TRUE
      )

# US county map of climate change opinion

legend_title <− ""
ggplot(data = cc_belief_county) + geom_polygon(aes(long, lat, group = group,
  fill = human2018), color = "grey", size = 0.1) + 
  coord_map("bonne", parameters = 41.6) +
  scale_fill_gradient(legend_title, low= 'light blue’, high= 'hot pink’) +
